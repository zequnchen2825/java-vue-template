<template>
  <div id="hxy-editor-20200819">
    <div id="xhy-editor"></div>
...
export default {
  ...
  methods: {
    getContent() {
      console.log(String(this.editContent).trim())
    },
    init2(menus) {
      // 官网 wangeditor: "^3.1.1" 不提供清除编辑器的方法 暴力处理
      let parantDom = document.getElementById('hxy-editor-20200819');
      let node = parantDom.childNodes[0]
      if(node) {
        parantDom.removeChild(node)
      }
      let newDom = document.createElement('div')
      newDom.setAttribute('id', 'xhy-editor');
      parantDom.appendChild(newDom)
      let editor = new E(newDom);
      this.editor = editor;
    ...
    init() {
    	let _this = this;
      let editor = new E(document.getElementById('xhy-editor'));
      this.editor = editor;
	    editor.customConfig.menus = menus;
	    editor.customConfig.emotions = emotions;
	    //开启debug模式
